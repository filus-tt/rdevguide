...
\chapter{Diálogo entre Filus y Aisa}\label{sec-dialogo}

Este es el texto narrativo normal del libro, que usa la tipografía
principal.

\section{Método 1: Usando comandos LaTeX
directos}\label{muxe9todo-1-usando-comandos-latex-directos}
...
\section{Método 2: Usando entornos LaTeX para diálogos
largos}\label{muxe9todo-2-usando-entornos-latex-para-diuxe1logos-largos}
...
\section{Ejemplos de código}\label{ejemplos-de-cuxf3digo}

Aquí hay un ejemplo de código en R, que usa la tipografía monoespaciada:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Función de ejemplo}
\NormalTok{calcular\_suma }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(a, b) \{}
...
\NormalTok{total }\OtherTok{\textless{}{-}} \FunctionTok{calcular\_suma}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{3}\NormalTok{)}
\FunctionTok{print}\NormalTok{(total)}
\end{Highlighting}
\end{Shaded}

\section{Texto normal}\label{texto-normal}

Después de los diálogos, el texto vuelve a la tipografía normal del
libro. Esto mantiene una jerarquía visual clara en todo el documento.
...
\chapter{titulo Filus}\label{titulo-filus}

\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, leftrule=.75mm, colframe=quarto-callout-color-frame, toprule=.15mm, arc=.35mm, breakable, left=2mm, opacityback=0, rightrule=.15mm, colback=white]
